...
const char *sw_version = "20211024";
...
void adjustFan()
{
  if (systemTemperature > maxAllowedBoardTemp * .75)
  {
    if (fanPWMVal < maxPWMVal)
    {
      ...
      if (debugPWM)
      {
        Serial.print("FAN: SPEED INCREASING - TEMPS APPROACHING LIMITS - PWM value: ");
...
void ioControl()
{
  ...
  if ((!currentLimited) || (!temperatureLimited)) // all systems report normal opearation
  {
    if ((battery1Voltage > 12.40) && (battery1Voltage < 12.70) && (!carStarting)) // B1 in normal state (B1 disconnected)
    {
      // B1 control
    ...
    if ((battery2Voltage > 11.50) && (battery2Voltage < 12.9)) // B1 voltage high or B2 voltage normal (normal run mode)
    {
      // B1 control
    ...
    if ((battery1Voltage > 12.70)) // B1 charging
    {
      // B1 control
